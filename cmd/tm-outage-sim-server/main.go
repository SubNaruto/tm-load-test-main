...
func main() {
	var (
		addr         = flag.String("addr", ":26680", "the address to which to bind this server")//服务器将侦听的地址和端口
		username     = flag.String("u", "tm-load-test", "the username for requests authenticating against the control endpoint")//对控制端点进行身份验证的用户名
		passwordHash = flag.String("p", "", "a bcrypt password hash for authenticating requests to the control endpoint")//验证对控制端点的密码哈希
	)
	flag.Usage = func() {
...
Examples of how to bring Tendermint up/down:
  ...
  curl -s -X POST -d "down" http://tm-load-test:testpassword@127.0.0.1:26680`)
	...
	http.HandleFunc(
		"/",
		outagesim.MakeOutageEndpointHandler(//创建HTTP处理函数，传入请求
			*username,//验证
			strings.Replace(*passwordHash, "#", "$", -1),//验证
			outagesim.IsTendermintRunning,//检查Tendermint是否正在运行
			outagesim.ExecuteServiceCmd,//执行服务命令
		),
	)
	log.Printf("Starting Tendermint outage simulator server at %s\n", *addr)//启动
	log.Fatal(http.ListenAndServe(*addr, nil))//日志记录
}
